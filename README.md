...
> Evidence-backed Supply Chain Intelligence Graph Alpha
> 证据优先的供应链情报图谱 alpha

SupplyStrata 是一个开源的**证据优先供应链情报图谱 alpha**：它把公司披露、供应商名单、实体注册信息，以及逐步接入的物流与贸易观测整理成可审计、可追溯、可重建的供应链关系网络。

SupplyStrata 把公开信息变成可验证的研究资产：每条边都有证据等级、原文片段、来源链接、抓取时间、置信度、review 状态和 unknown map。当前最准确的定位是：面向关键技术产业链研究的 evidence-backed workbench，而不是已经覆盖全球所有行业、实时货物流和风险告警的综合情报产品。

---

## 本项目是什么

把**公开数据**（公司 10-K / 年报 / IR / 监管文件；后续逐步扩展到海关贸易、港口 AIS、能源商品价格、政府采购等 observation / lead）整理为一张可追溯、可视化的供应链证据图谱。
...
它面向产业链研究、决策支持、金融研究、采购分析和开源情报工作流，目标是把"NVIDIA 上游有谁、HBM 供给紧张到什么程度、Apple 哪些组件来自哪些地区"这类问题，整理成**有证据、有时间戳、有来源、有不确定性标注**的答案。全球级监控、动态风险提示和货物流向综合推断属于后端完成 gate 之后的产品化目标。

具体定位、边界和术语请看 [docs/00-overview/overview.md](./docs/00-overview/overview.md)。
...
1. [overview.md](./docs/00-overview/overview.md) — 定位、边界和术语
2. [intelligence-methodology.md](./docs/03-data-model/intelligence-methodology.md) — 情报方法学边界
3. [backend-completion-criteria.md](./docs/06-development/backend-completion-criteria.md) — 后端完成门槛
4. [module-design.md](./docs/02-architecture/module-design.md) — 模块拆分与接口契约
5. [data-flow.md](./docs/02-architecture/data-flow.md) — 主数据流
6. [schema.md](./docs/03-data-model/schema.md) — truth store / derived view schema
7. [source-registry.md](./docs/04-data-sources/source-registry.md) — 数据源权威表
8. [quickstart.md](./docs/06-development/quickstart.md) — 从空环境跑到研究输出
...
4. **规则先行，LLM 兜底**。LLM 产出候选并进入 review 流程；cite_text 校验和 `evidence_level = 4` 上限按 [decisions.md](./docs/10-decisions/decisions.md) 执行。
...
| Research pack / ChainView           | implemented first backend export path                                    |
| Source monitor / worker             | Postgres-backed source_check_jobs baseline implemented                   |
| Risk / intelligence context         | deterministic baseline implemented; backend completion gates still open  |
...
`preview` 命令执行 source adapter `plan/fetch/normalize`、规则抽取、seed 实体消歧和证据评分；适合未来嵌入 TS 桌面端或 agent 产品。当前切片走规则抽取路径，LLM 扩展遵循 [decisions.md](./docs/10-decisions/decisions.md)。
...
LLM 策略、`needs_review` 默认值与 cite_text 校验规则按 [decisions.md](./docs/10-decisions/decisions.md) 保留，当前可运行切片使用规则抽取。
...
本仓库保存代码、文档、配置样例和项目维护的 seed metadata。`data/`、`reports/`、`.env` 保持在本地；公开输出保留必要 cite_text 片段、来源 URL 和来源日期。第三方数据源许可与 ToS 见 [compliance.md](./docs/09-risks-compliance/compliance.md)。
...
后续方向只看 [docs/06-development/backend-completion-criteria.md](./docs/06-development/backend-completion-criteria.md)。旧路线图已经删除，避免和当前 gate 口径冲突。
