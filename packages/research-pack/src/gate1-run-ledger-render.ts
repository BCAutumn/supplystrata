...
export function renderGate1RunLedgerMarkdown(ledger: Gate1RunLedger): string {
  const lines = [
    ...
    "",
    `- Layers: ${ledger.propagation_execution.summary.layers}`,
    `- Queue items: ${ledger.propagation_execution.summary.queue_items}`,
    `- Run source target: ${ledger.propagation_execution.summary.run_source_target}`,
    `- Repair source target: ${ledger.propagation_execution.summary.repair_source_target}`,
    `- Review intelligence context: ${ledger.propagation_execution.summary.review_intelligence_context}`,
    `- Keep unknown open: ${ledger.propagation_execution.summary.keep_unknown_open}`,
    `- Runnable source targets: ${ledger.propagation_execution.summary.runnable_source_targets}`,
    `- Blocked source targets: ${ledger.propagation_execution.summary.blocked_source_targets}`,
    `- Next focus: ${ledger.propagation_execution.next_focus}`,
    "",
    "## Monitoring Config",
    "",
  ...
  for (const batch of ledger.monitoring_config.batches) {
    lines.push(
      `- ${batch.batch_id}: ${batch.target_count} targets, state=${batch.current_state}, next=${batch.recommended_next_decision}, action=${batch.recommended_operational_action}`
    );
