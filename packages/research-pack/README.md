...
- 把 L4/L5 fact edge 增长、counterparty corroboration、source blocker、strength 缺口、observation labeling batch 和 propagation context 缺口汇总成只读数据深度工作台，并拆成前端/host app 可消费的 action batch。每个 action item 都带推荐决策、允许决策、写入影响和命令提示，帮助下一轮跑数和算法校准聚焦高价值缺口。
