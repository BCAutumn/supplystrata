...
export async function seedFromCsv(client: DbClient, rootDir = process.cwd()): Promise<{ entities: number; aliases: number; components: number }> {
  ...
  for (const row of entities) {
    ...
    if (row.tickers.trim().length > 0) identifiers["ticker"] = row.tickers.split(";").map((ticker) => ticker.trim()).filter(Boolean);
    await client.query(
      ...
      [
        ...
        row.industry.split(";").map((item) => item.trim()).filter(Boolean),
        row.status,
  ...
  for (const row of components) {
    await client.query(
      ...
      [
        row.component_id,
        row.name,
        row.taxonomy_path.split(">").map((part) => part.trim()).filter(Boolean),
        row.aliases.split(";").map((part) => part.trim()).filter(Boolean)
      ]
    );
  }
...
function autoAliasId(entityId: string, alias: string): string {
  const suffix = normalizeAlias(alias).replace(/[^a-z0-9]+/g, "-").replace(/(^-|-$)/g, "").slice(0, 64);
...
export async function recordPendingEntity(client: DbClient, input: { surface: string; context: Record<string, unknown> }): Promise<{ pending_id: string; is_new: boolean }> {
  ...
  if (current !== undefined) {
    await client.query("UPDATE pending_entities SET occurrence_count = occurrence_count + 1, context = $2 WHERE pending_id = $1", [current.pending_id, input.context]);
...
async function ensureDefaultUnknownItems(client: DbClient): Promise<void> {
  ...
  for (const question of questions) {
    await client.query(
      ...
      [`UNK-NVIDIA-${normalizeAlias(question).replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "").toUpperCase()}`, question]
    );
  }
}
...
export async function resolveEntityId(client: DbClient, input: string): Promise<string> {
  ...
  const aliasResult = await client.query<{ entity_id: string } & pg.QueryResultRow>(
    "SELECT entity_id FROM entity_alias WHERE alias_norm = $1 LIMIT 1",
    [normalized]
  );
