...
export const skHynixIrAdapter: SourceAdapter<SkHynixIrInput, Uint8Array> = {
  ...
  async fetch(task, ctx) {
    ...
    const storageKey = `company-ir/skhynix/${year}/${sha256}.html`;
...
async function fetchOrLoadCached(url: string, userAgent: string, year: string): Promise<Uint8Array> {
  try {
    return await fetchBytesWithTimeout(url, { userAgent, timeoutMs: 12_000, sourceLabel: "SK hynix disclosure" });
  } catch (error) {
...
async function readLatestCached(year: string): Promise<Uint8Array | undefined> {
  const dir = join(loadEnv().OBJECT_STORE_FS_BASE, "company-ir", "skhynix", year);
  try {
