...
export const TWSE_MOPS_DOCUMENT_KINDS = ["F", "A", "K"] as const;
...
export function normalizeTwseMopsElectronicDocuments(raw: RawDocument<Uint8Array>): NormalizedDocument {
...
export function twseMopsElectronicDocumentsTaskMetadata(task: FetchTask, html: string): Record<string, unknown> {
...
export function validateTwseMopsElectronicDocumentsInput(input: TwseMopsElectronicDocumentsInput): void {
...
export function decodeTwseMopsHtml(bytes: Uint8Array): string {
...
export function assertTwseMopsResponse(html: string, sourceUrl: string): void {
  if (html.includes("FOR SECURITY REASONS") || html.includes("安全性考量")) throw new Error(`TWSE MOPS security page returned for ${sourceUrl}`);
}

export function isTwseMopsDocumentKind(value: string): value is TwseMopsDocumentKind {
