...
│   ├── gate1-supplier-list-batch [--reviewer] [--limit] [--apply]
│   ├── gate1-entity-source-batch [--reviewer] [--limit] [--apply]
│   └── enqueue
│       ├── apple-suppliers
│       ├── entity-source <query> [--source] [--jurisdiction] [--limit]
│       └── gate1-supplier-entity-backlog [--source] [--scan-limit] [--supplier-limit] [--candidate-limit]
├── graph
│   ├── rebuild
│   ├── check [--format]
...
导出 `apps/research-preview` 消费的本地 JSON。这个命令由 `@supplystrata/workbench-export` 组装数据，CLI 只负责参数解析和写文件。JSON 包含 `companies / chain_segments / edges / claims / draft_claims / evidences / unknown_items / sources / changes`，前端不得直连数据库补字段。`unknown_items` 会包含 `scope_kind / scope_id`，供 research-pack 和后续 agent 按结构化归属识别 edge/claim/company unknown，而不是解析问题文案。`draft_claims` 只用于侧栏研究草稿，不属于 ChainView 主链路。`evidences` 会包含 ChainView 事实边上的全部 evidence 和 superseded evidence，方便工作台做证据审计，而不是只展示 primary evidence。
...
--target-profile <id|none> 可选；研究目标 profile；公司或组件命中内置 profile scope 时自动选择，传 none 可关闭
...
--prepare-data             显式刷新 claims、edge intelligence、root unknown 和 eligible component risk 后再导出
--build-claims             只显式刷新 active claims
--refresh-intelligence     只显式刷新 edge strength/freshness/strength unknown
--refresh-component-risk   只显式刷新 eligible component risk baseline
--skip-claims              搭配 --prepare-data 时跳过 active claims
--skip-intelligence-refresh 搭配 --prepare-data 时跳过 edge strength/freshness/strength unknown
--skip-component-risk-refresh 搭配 --prepare-data 时跳过 eligible component risk baseline
--skip-root-unknowns       搭配 --prepare-data 时跳过 selected company root unknown materialization
--out <dir>                输出目录，默认 reports/research-pack
```

把已有 truth-store 数据打包成可复现研究包。它不接新源、不联网抓取、不写图谱事实边；默认也不写 claims、edge intelligence、root unknown 或 risk views。需要导出前刷新派生上下文时，必须显式传 `--prepare-data` 或单独传 `--build-claims / --refresh-intelligence / --materialize-root-unknowns / --refresh-component-risk`。root unknown 只用于 selected company / parent legal-entity 研究入口没有任何 reviewed L4/L5 fact edge 且没有 open company unknown 的情况；它只写 `unknown_items` 和 semantic change，不写 evidence 或 fact edge。随后导出 `workbench.json`、CompanyCard、ChainView、ComponentCard、source plan、data-quality report、question readiness matrix、official disclosure readiness、Gate 1 run ledger、investigation backlog、source target coverage 和 manifest。`--source-target-preflight` 只读取显式传入的 smoke JSON，并把它写成 `source-target-preflight.json/md`；research-pack 不会因此重新访问外部源。这个命令由 `@supplystrata/research-pack` 负责，CLI 只做参数解析和写目录。

`question-readiness.json/md` 只回答“当前数据够不够回答核心问题”，不会生成自然语言结论，也不会把缺口补成事实。`official-disclosure-readiness.json/md` 把 Gate 1 相关的研究目标 profile、逐节点覆盖状态、逐 expected source 覆盖状态、Level 4/5 fact edge 数、完整 traceability、严格 cross-source corroboration、strength/freshness 覆盖、explicit unknown、profile expansion candidates 和官方披露 source target 状态量化；它不会把 single-source silence 自动解释为已审计，也不会写事实边。对没有二源路径且没有 linked disposition unknown 的 single-source edge，报告会给出确定性 `proposed_unknown`；这个 payload 本身不是事实证据，也不是已持久化 unknown。需要落库时必须走 `supplystrata intelligence single-source-unknowns`，由 `evidence-maintenance` 默认确认目标 edge 仍为 current 后写入 `unknown_items`。内置 `ai-compute-memory.v0` 是验收锚点，不是全球供应链全集；不在 profile 中但被事实边或官方 source-plan 发现的节点会进入 `profile_expansion` backlog，等待审阅是否纳入 profile。expected source 覆盖会把 profile 期待来源拆成 `covered_fact / official_target_synced / official_target_runnable / official_source_planned / connector_available / source_registered_unimplemented / missing_source_mapping` 等状态，避免把“来源清单”误解成“已经接通监控”。research-pack 会把 target profile hints 下沉给 source-plan：已有 CIK 的 SEC 公司生成 `sec-company-filings` 和 observation-only `sec-company-facts` runnable suggestion，已有官方 IR connector 的来源在显式年份存在时生成 node-specific `official-html-disclosure` suggestion，带审计 HTTPS URL 的 `company-ir` 目标生成受控长尾 IR suggestion，DART / EDINET / TWSE 监管目录 connector 在显式年份存在时生成目录 monitor suggestion；缺显式 URL、公司级代码、connector 或 config 的来源仍是缺口。`investigation-backlog.json/md` 把 readiness gap、official disclosure gap、expected source gap、profile expansion candidate、explicit unknown、组件覆盖缺口和 source-plan item 汇总成下一步调查任务，供人工或未来安全 agent 消费；它不运行 adapter，也不写 DB。`corroboration-source-plan.json/md` 会把逐 edge `corroboration_review` 的 runnable target 过滤成标准 source-plan 子集，可直接交给 `sources policy preview-plan-targets / smoke-plan-targets / sync-plan-targets / enable-plan-targets`；每个 target ref 会带 `next_action / next_action_reason`，说明当前应先配置凭据、修配置、smoke、sync、enable、run due、等待 job、排查失败还是 review observation，manifest 和 README 也会汇总 next-action 分布；当某类可执行 next action 非空时，还会额外输出 `corroboration-source-plan-smoke.json`、`corroboration-source-plan-sync.json`、`corroboration-source-plan-enable.json` 或 `corroboration-source-plan-run-due.json`，让执行批次按已审计动作拆开，避免跳过 smoke 或把缺凭据目标混进同步/启用。`gate1-run-ledger.json/md` 会把 readiness、corroboration-source-plan、source-target coverage 和 supply-chain expansion plan 合并成一个执行账本：它给出当前 mainline phase、data/source path 进度、可执行命令提示、single-source unknown materialization 提醒，以及下一层 counterparty 的通用 `research run --company ... --component ...` 建议。这个账本只读，不抓源、不写库、不生成事实边；其中全量官方 source path 同步指向 `source-plan.json`，edge-level 二源检查仍指向 `corroboration-source-plan*.json`。账本里的 `monitoring_config` 是给后续前端/host app 的统一监控配置面：它列出 namespace、默认 cadence / jitter / retry / backoff / `next_check_at`、可配置字段和 source-plan 批次建议，前端可以直接把这些字段映射成表单或审批动作；这些字段只影响 source policy/target 调度，不代表事实结论。每个 monitoring batch 还会带 `state_counts`、`recommended_operational_action` 和 `attention_hint`，把 disabled、due、retry_wait、degraded、dead、source_failed、observation、preflight missing credentials / invalid config / unreachable 以及 DB-backed job failure kind 分开，避免前端只给一个“继续跑”的模糊按钮。账本里的 `review_workbench` 是给后续前端/host app 直接消费的审查入口：每个 item 都带推荐决策、允许决策、命令提示、引用对象、写入影响和 `automatic_fact_mutation_allowed=false` policy；规则可以自动排序和准备命令，但事实层变更仍必须通过独立受控 use-case。`source-target-preflight` 的 issue kind 分布也会进入 manifest / README，方便直接看到 Gate 1 卡在缺凭据、配置错误、connector 未实现、源不可达还是 adapter 异常。它只缩小执行批次，不生成二源事实。`source-target-coverage.json/md` 把 runnable source-plan target 与 `source_check_targets / source_check_jobs / source_change_events / observations` 对齐，显示 target 是否已同步、启用、due、运行、重试、degraded、dead 或已经产出 observation；失败 job 会带 `failure_kind`，metric 覆盖还会生成 P0/P1/P2 的 `observation_review` seeds，并显式标记 review-only/no fact mutation，方便前端或宿主 App 做指标审查和 gold label 抽样。backlog 会消费 coverage 状态，把 action 写成“先同步 / 启用 / 跑 due / 等待 active job / 排查 failed/degraded job / review observation”这类可执行下一步。component risk refresh 只覆盖有可审计 fact edge 的 eligible 组件；只有 taxonomy 或 source-plan 的组件会继续显示为 coverage gap。

如果 `official-disclosure-readiness` 里存在 open official signal correlation hints，`gate1-run-ledger.action_queue` 会额外输出 `record_official_signal_dispositions`。它让 CLI、未来前端或宿主 App 可以用同一个 action queue 暴露 signal disposition 审查入口，而不用各自解析 readiness 明细；该入口仍然只是 review-only，不写 evidence、fact edge 或 source target。
...
supplystrata intelligence observation-calibration-label OBS-EXAMPLE --candidate observation-calibration:purchase_obligations:OBS-EXAMPLE --label useful_signal --reviewer analyst
supplystrata intelligence ranking-calibration-label ranking:adjacent-company:COMP-PCB:adjacent-company-ranking.v1 --candidate ENT-IBIDEN --rank 1 --label useful_target --reviewer analyst
```

`calibration-label` 写 fact edge 人工 gold label；`calibration-run` 只从这些 label 计算 precision、reliability buckets 和 error summary。`observation-calibration-label` 只写 observation gold label 样本池。`ranking-calibration-label` 只写 research target ranking 样本池，用于记录候选是否 useful、方向是否错、是否存在品牌/披露中心偏差、是否需要更多上下文或无关。三者都不写事实边，不改变 evidence level，也不替代 review/apply。

### supplystrata intelligence single-source-unknowns

```bash
supplystrata intelligence single-source-unknowns --readiness reports/nvidia-research-pack/official-disclosure-readiness.json
```

把 `official-disclosure-readiness.json` 里确定性的 `proposed_unknown` 受控写入 `unknown_items`，用于把 single-source L4/L5 fact edge 的二源处置缺口变成显式 backlog。业务编排在 `@supplystrata/evidence-maintenance`，CLI 只负责读取文件和输出 summary。默认会确认目标 edge 仍为 `current`；`--allow-missing-edges` 只用于迁移或测试，不应用来绕过事实层生命周期。该命令不写 `edges`，不改变 `evidence_level`，也不把单源沉默解释成 corroboration。

### supplystrata intelligence official-signal-unknowns

```bash
supplystrata intelligence official-signal-unknowns --review REV-OFFICIAL-SIGNAL-EXAMPLE
supplystrata intelligence official-signal-unknowns --edge EDGE-EXAMPLE --limit 50
```

把已记录的 official disclosure signal disposition 中 `decision='record_single_source_unknown'` 的项物化为 edge-scoped `unknown_items`。它只读取 `review-store` 写下的审计 change，默认确认目标 edge 仍为 `current`，并复用 `evidence-maintenance` 的 unknown 写入和 semantic change 审计。它不读取 signal 自行判断，不写 evidence，不写 fact edge，也不会把 `supports_existing_edge` disposition 自动升级成 corroboration。
...
--source <ids>                    可选；只预览这些 source adapter，逗号分隔
...
无数据库审计 `research run` / `research from-workbench` 输出的 runnable `suggested_check_targets`。转换和校验都在 `@supplystrata/source-management` 内完成；CLI 只渲染结果。预览会输出稳定 namespace、将生成的 `check_target_id`、去重后的 target 数、按 source / target kind / priority 的统计、需要凭据的 target 数，以及同一套 source-management validation error/warning。`--source` 可把全量计划收窄到当前已审计或不需要凭据的来源，避免把缺 key 的目标混进本次同步/启用批次。它不写 `source_check_targets`，也不抓源；适合在宿主 App 或命令行里先确认这批监控目标是否值得同步。
...
supplystrata review enqueue gate1-supplier-entity-backlog [--source gleif] [--scan-limit N] [--supplier-limit N] [--candidate-limit N]
...
supplystrata review gate1-supplier-list-batch --reviewer <name> [--limit N] [--apply]
supplystrata review gate1-entity-source-batch --reviewer <name> [--limit N] [--apply]
supplystrata review entity-affiliation-disposition <context-id> --subject <ENTITY-id> --parent <ENTITY-id> --decision research_parent_entity --reviewer <name> --reason "..."
supplystrata review signal-disposition <REV-id> --edge <EDGE-id> --decision needs_more_evidence --reviewer <name> --reason "..."
```
...
- `official_disclosure_signal`：官方披露文档里的供应链/产能/需求/技术路线信号。`review apply` 只 acknowledge，不写 edge/evidence。要记录它对某条 edge 的审阅结论，使用 `review signal-disposition`，decision 只能是 `supports_existing_edge`、`needs_more_evidence`、`not_relevant`、`record_single_source_unknown` 或 `create_counterparty_source_target`；该命令只写 `OFFICIAL_DISCLOSURE_SIGNAL_DISPOSITION_RECORDED` change，且 `fact_write_policy.automatic_fact_mutation_allowed=false`。如果 decision 是 `record_single_source_unknown`，后续再用 `intelligence official-signal-unknowns` 受控物化 unknown。

`review entity-affiliation-disposition` 用来记录 Gate 1 entity context 的审阅结论，例如 Samsung Memory 这类业务单元是否应该转到 Samsung Electronics 父级法人继续跑递归研究。decision 只能是 `research_parent_entity`、`research_child_entity`、`research_both_scopes`、`not_relevant` 或 `keep_unknown_open`；命令只写 `ENTITY_AFFILIATION_DISPOSITION_RECORDED` change，且 `fact_write_policy.automatic_fact_mutation_allowed=false`。它不合并实体、不继承父级 evidence、不写 fact edge，也不关闭 unknown。后续 research-pack 会读回最新 disposition，用它决定是否仍需要前端审查、是否可以把父级法人作为递归 research target。
...
`review gate1-supplier-list-batch` 是 Gate 1 的受控批处理入口，默认 dry-run，只扫描 pending Apple Supplier List 候选，检查官方来源行、curated supplier entity 解析和可复现 citation 是否齐备。输出里的 `entity_resolution_backlog` 会聚合最该先补的 unresolved supplier。加 `--apply` 后，它仍然先 approve 再调用同一条 `applyApprovedReviewCandidate()` 路径，不绕过人工审查状态机、不直接写 fact edge；被跳过的候选通常需要先补实体解析、修 source row/citation 或保留为 unknown/disposition。

`review enqueue gate1-supplier-entity-backlog` 会先用同一套 Gate 1 supplier-list dry-run 生成 `entity_resolution_backlog`，再把前 N 个 supplier 名称送入 entity-source review 队列。默认源是无需 API key 的 GLEIF；命令只 enqueue 外部实体候选，不 merge `entity_master`，也不重试 supplier-list apply。

`review gate1-entity-source-batch` 是 backlog 后的受控实体合并入口，默认 dry-run，只扫描 pending `entity_source_candidate`。它目前只允许 GLEIF、`ACTIVE`、`corroboration=FULLY_CORROBORATED` 且 supplier surface 与 GLEIF legal name 精确匹配的候选进入批量 approve/apply；其它候选仍留给人工 review 或后续 source corroboration。加 `--apply` 后，它仍走同一条 `review approve -> applyApprovedReviewCandidate()` 审计路径，只补 `entity_master/entity_alias/pending_entities`，不会直接生成供应链事实边。实体合并后再重新运行 `review gate1-supplier-list-batch --apply`，由 supplier-list 规则释放对应 L4/L5 fact edge。
