...
把已有 truth-store 数据打包成可复现研究包。它不接新源、不联网抓取、不写图谱事实边；只刷新 claim 层（默认）、导出 `workbench.json`、CompanyCard、ChainView、ComponentCard、source plan、data-quality report 和 manifest。这个命令由 `@supplystrata/research-pack` 负责，CLI 只做参数解析和写目录。

示例：

```bash
supplystrata research run --company nvidia --component COMP-HBM,COMP-MEMORY --trade-month 2025-12 --commodity-month 2025-12 --out reports/nvidia-research-pack
```
