...
├── patterns/sec-official-supply-chain.json   # counterparty / component / service supplier 规格
└── src/
    ├── pattern-catalog.ts                    # JSON 读取与严格校验
    ├── patterns.ts                           # 把规格编译成 RegExp
    └── index.ts                              # 抽取流程
```

`pattern-catalog.ts` 不使用 `any`，会在模块加载时校验 JSON 结构、组件粒度、共享组件 key 和正则 source 字段。新增 counterparty / component 时优先改 JSON；只有新增一种抽取语义时才改 `index.ts`。
