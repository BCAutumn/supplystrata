...
```text
Comtrade / EIA / FRED / World Bank / NOAA AIS / GDELT
```

这些源很重要，但对 v0.2 默认事实边贡献低，应该先等 observation schema 和 ChainView 契约落地。
...
Comtrade、EIA、NOAA AIS 很重要，但它们不能直接回答公司级关系。先建 observation schema，再接源，避免把宏观信号污染事实图。
