...
`company-ir` 是一个**逻辑** source family。当前已实现的官方 IR HTML adapter 收敛在 `source-workflows` 的 official IR 能力内，避免为每个只含 URL/normalize/context 的薄壳单独建包；同时提供一个 `company-ir/official-html-disclosure` 受控入口，只接受已审阅的显式 HTTPS URL，不做自动发现。
...
每家 IR 网站结构不同；不写“猜任意公司 IR 页”的通用抓取器。P0/P1 公司可以保留独立 adapter id、URL 规则和限速；长尾公司只有在 profile、review 或 host app 明确给出 `url + entity_id + year` 后，才进入 `company-ir/official-html-disclosure`。
