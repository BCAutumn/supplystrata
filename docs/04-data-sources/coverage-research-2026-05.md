...
| `sec-edgar`       | `sec-company-facts`           | observation |
| `company-ir`      | `official-html-disclosure`    | signal/edge |
...
| `micron-ir`       | `official-html-disclosure`    | signal/edge |
| `asml-ir`         | `official-html-disclosure`    | signal/edge |
| `apple-suppliers` | `supplier-list-review`        | review/lead |
| `dart-kr`         | `company-filings`             | signal/edge |
| `edinet`          | `daily-filings`               | signal/edge |
| `twse-mops`       | `electronic-documents`        | signal/edge |
...
### 3. EDINET：日本官方披露目录监控 / 后续事实边

**建议优先级：高，DART 后做。当前已先接 daily-filings 目录 monitor。**

EDINET API v2 官方规格支持 documents list 和文档下载。第一步只使用 `documents.json` 做目录监控和 readiness/target 覆盖，适合覆盖 Tokyo Electron、Sony、Murata、Shin-Etsu、Renesas、Ibiden 等日本半导体/电子供应链节点的官方披露发现路径。事实边与实体/子公司上下文必须等 XBRL ZIP / PDF 正文解析、实体解析和 evidence scoring 补齐后再进入 review/apply。
...
- 已接目录 monitor；后续仍需要处理日文文档和 XBRL/PDF 两种正文路径。
