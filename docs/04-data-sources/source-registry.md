...
| `seed-entities`         | -    | （已移除）dev-only entity fixtures             | 仅 CI / 本地开发 fixture；产品默认覆盖走 registry bootstrap | -                                        | `tests/fixtures/dev-entities/`        | -                 | removed     |
...
| `wikidata`              | P1   | Wikidata SPARQL / EntityData                   | 公司公开简介、行业、官网 URL、跨标识符链接                  | 用于 entity-resolution + dynamic profile | 官方 SPARQL/REST                      | CC0 + 归因        | preview     |
...
- `planned`：MVP 范围内
- `scoped`：已规划但未接入
- `manual_only`：只允许人工录入或人工触发
- `removed`：已从产品默认覆盖路径移除，只保留历史说明或 dev fixture trace
- `rejected`：审视后明确不接
...
| `gleif`                                                           | `government_registry`      | `registry_fact`    | 4                  | 全球 LEI 身份上游，用于 entity bootstrap，不作为供应链关系证据。                               |
| `openfigi`                                                        | `commercial_registry`      | `registry_fact`    | 4                  | 全球金融工具身份上游；FIGI 不是法人登记，必须作为候选和其他来源交叉确认。                      |
| `wikidata`                                                        | `collaborative_registry`   | `registry_fact`    | 3                  | Collaborative identity graph；只能辅助消歧和跨标识符链接，不得升级成 Level 4/5 registry fact。 |
