...
先建 observation 表，并先接一个低风险的官方披露 observation extractor。它只从已经抓取/标准化的 SEC 或公司官方年报中抽取 inventory、backlog、capex、customer concentration、procurement commitment 等观测信号；这些信号进入 `observations`，不进入 `edges`，也不会被 graph-builder 当成事实边。

验收：

- official disclosure / trade / commodity / energy / port / route observations 有统一 provenance。
