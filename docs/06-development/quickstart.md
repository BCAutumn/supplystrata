...
再次运行同一个 `research run --source-target-namespace nvidia-memory-2025` 时，研究包会输出 `source-target-coverage.json/md`。这个文件把 runnable source-plan target 对齐到 `source_check_targets / source_check_jobs / source_change_events / observations`，显示目标是否已同步、是否启用、是否 due、是否 retry_wait、是否 degraded、最近 job/event 是什么，以及有没有产出 observation。summary 还会汇总 total observations、observed subject entities、observations by source、observations by target kind 和 observations by metric，用来判断数据是否真的跑深；`observation_review` 会把 metric 覆盖转成 P0/P1/P2 review seeds，并从样本生成只读 `calibration_candidates`。候选标签限定为 `useful_signal / background_context / needs_context / not_useful`，给前端/host app 做人工或规则审查、calibration / gold label 抽样；truth-store 模式会回填已落库的 `observation_calibration_labels`，在 candidate 上显示 `existing_labels / latest_label / review_status`，并在 manifest 里统计 labeled/unlabeled 和 persisted label 分布。`labeling_plan` 会从未标注 candidate 中按 priority 和 metric 分层抽取下一批建议样本，帮助扩大 gold set 时保持覆盖面。它们都带 `review_only_no_fact_mutation` policy，不会自动写 fact edge、risk 结论、observation 或 evidence level。失败 job 还会带 `failure_kind`，用于区分缺凭据、target config 错误、源不可达/响应错误、限流、adapter 错误或未知失败；它只是排障分类，不是事实证据。`investigation-backlog.md` 也会把这些状态写进每个相关任务的 action 和 coverage 行里，例如“已同步但未启用”时会提示先启用 target，“source fetch degraded” 时会提示先排查源退化；如果显式传入 `--source-target-preflight`，failed/skipped/degraded preflight 会优先提示先修 target config、凭据、connector 注册或源连通性，而不是笼统提示去同步 source。`gate1-data-depth-workbench.json/md` 会把 L4/L5 边增长、二源 corroboration、source blocker、strength 缺口、下一批 gold label 和 propagation context 缺口排成 review-only 优先级清单，方便本轮跑数后直接看下一步补哪里；它还会输出 `gate1-data-depth-p0.json`、`gate1-data-depth-source-blockers.json`、`gate1-data-depth-labeling.json`、`gate1-data-depth-corroboration.json` 和 `gate1-data-depth-intelligence-context.json` 等非空 action batch。每个 action item 都带 `frontend_action_kind`、推荐/允许决策、写入影响和命令提示，供前端/host app 分别展示优先审查、排障、标注、二源复核和推理上下文动作；这些 batch 仍不写事实层。`gate1-run-ledger.json/md` 的 `monitoring_config.batches[]` 会把这些状态汇总为 `state_counts`、`recommended_operational_action` 和 `attention_hint`，给未来前端/host app 直接生成同步、启用、运行、等待、排查失败或 review observation 的动作。它是监控闭环的可见性层，不会自动把 observation 升级成 fact edge。
...
只想从 truth store 做只读打包时，不需要额外参数；这是 `research run` 的默认行为。`--skip-claims`、`--skip-intelligence-refresh`、`--skip-component-risk-refresh` 只用于搭配 `--prepare-data`，在组合刷新里关闭某个写入步骤。
...
标注后再次生成 truth-store research pack，`source-target-coverage.json/md` 会在对应 candidate 上显示已持久化 label，并在 manifest 中更新 labeled/unlabeled 和 persisted label 分布；静态 snapshot 不连接数据库，因此不会回填这些持久化 label。
