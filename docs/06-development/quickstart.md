...
再次运行同一个 `research run --source-target-namespace nvidia-memory-2025` 时，研究包会输出 `source-target-coverage.json/md`。这个文件把 runnable source-plan target 对齐到 `source_check_targets / source_check_jobs / source_change_events / observations`，显示目标是否已同步、是否启用、是否 due、是否 retry_wait、是否 degraded、最近 job/event 是什么，以及有没有产出 observation。`investigation-backlog.md` 也会把这些状态写进每个相关任务的 action 和 coverage 行里，例如“已同步但未启用”时会提示先启用 target，“source fetch degraded” 时会提示先排查源退化；如果显式传入 `--source-target-preflight`，failed/skipped/degraded preflight 会优先提示先修 target config、凭据、connector 注册或源连通性，而不是笼统提示去同步 source。`gate1-run-ledger.json/md` 的 `monitoring_config.batches[]` 会把这些状态汇总为 `state_counts`、`recommended_operational_action` 和 `attention_hint`，给未来前端/host app 直接生成同步、启用、运行、等待、排查失败或 review observation 的动作。它是监控闭环的可见性层，不会自动把 observation 升级成 fact edge。
