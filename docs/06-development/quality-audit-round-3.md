...
```text
[ ] F1-F6 DB Row / DTO 边界继续收敛：`loadEvidenceCard()` 已改为显式 EvidenceCard DTO 映射，`db/query.ts` 的 evidence 查询已去掉 `ev.*`；`WorkbenchModel.changes` 已改为本地 `WorkbenchChangeTimelineItem` DTO 并通过显式 mapper 从 db timeline 转换；card-builder 的 `UnknownItemRow -> UnknownMapItem` 映射已保留 `scope_kind/scope_id`，Unknown markdown/JSON 输出可审计归属；chain-view-builder、documents/chunk 查询和其他 Row 暴露面仍需继续治理。
