...
[ ] G1-G8 长 if 链继续按 facts+rules / registry / table-driven 模式收敛，优先参考 `claim-conflict` 的规则表。G1/G2/G3/G7 局部收敛：official disclosure signal extractor 用 source adapter registry 替代并列 source 分支；relation sentence extractor 不再默认 `ENT-NVIDIA`，调用方必须显式传 subject，foundry / memory supplier 触发逻辑改为 counterparty 规则表；edge claim type / claim text 改为 relation 映射表，`reviewStats()` 用 status -> stats key 映射表替代并列状态分支。
