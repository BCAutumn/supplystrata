...
[ ] K1/K2 clock / cwd 等隐式依赖需要逐步改为执行层显式注入，尤其是写入审计时间和批量任务。K2 已局部收敛：`SeedEntityResolver.fromCsv()`、`seedFromCsv()` 不再默认读取 `process.cwd()`，由 CLI / test / workflow runtime 显式传入 seed root。K1 已局部收敛：semantic-change claim draft 不再默认 `new Date()`，必须使用 review lifecycle 的 `reviewed_at`；缺失该时间时 apply 会 block 候选；`buildWorkbenchModel()` 支持显式 `generatedAt`，并用同一个时点驱动 export metadata、edge freshness 和默认 change timeline window，研究快照可重复。
```
