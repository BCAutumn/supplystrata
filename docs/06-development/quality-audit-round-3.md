...
```text
[ ] 多处 N+1 写入后续可改批量 SQL 或 unnest。`replaceRiskView()` 已将 `risk_metrics` 写入从逐条 insert 改为单次 `jsonb_to_recordset` 批量插入，component risk / anomaly / financial peer refresh 不再按 metric 数量放大写入语句数。
