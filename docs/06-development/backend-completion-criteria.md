...
[ ] AI compute propagation matrix 和 Gate 1 propagation action 必须输出 `readiness_answers`，把每层七个核心问题整理为 facts / non-fact inputs / official gaps / unknowns / next research / source target state / output policy
...
[x] AI compute propagation matrix 和 Gate 1 propagation workbench item 能输出 `readiness_answers`，把每层七个核心问题前置成 facts / non-fact inputs / official gaps / unknowns / next research / source target state / output policy
