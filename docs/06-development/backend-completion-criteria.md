...
- `gate1-data-depth-workbench` 会把 `official_evidence_gaps` 接进行动闭环：即使 layer 状态是 `covered_fact`，只要层内仍有 component/material/process/source gap，也会生成 `review_intelligence_context` item，并在 summary 中统计 gap 总数和 by-kind 分布。propagation workbench item 会携带同一层的 `evidence_layer_summary`，让 `gate1-data-depth-intelligence-context.json` 自身就能说明哪些是 fact、observation、lead、unknown、source target 或 official gap，以及哪些 truth-store 写入被禁止。这样 Gate 1 不会把“有一个 L4/L5 anchor”误认为“该层已经完整结束”。
