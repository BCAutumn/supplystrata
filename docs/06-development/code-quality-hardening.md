...
[ ] `DatabaseStore extends DbClient` 仍让“普通连接”和“事务连接”可在部分函数签名中混用；已覆盖 intelligence / single-source disposition / claim-builder / review-store / risk / alert 关键多写入链路，后续继续覆盖 calibration、graph projection job status、chain view build 等剩余写路径，并逐步迁移旧 import 到 read/write/admin 子路径。
