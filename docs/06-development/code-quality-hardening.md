...
[ ] `DatabaseStore extends DbClient` 仍让“普通连接”和“事务连接”可在部分函数签名中混用；已覆盖 intelligence / single-source disposition / claim-builder / review-store 关键多写入链路，后续继续覆盖 risk/alert 等写路径。
