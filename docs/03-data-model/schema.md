...
research-pack / Workbench 输出不是 PostgreSQL truth-store schema，但它们是前端、host app 和未来安全 AI 的稳定消费契约。尤其是 `ai_compute_propagation.v0` 的 `readiness_answers` 与 `execution_queue`：

- `readiness_answers` 只回答每个 propagation layer 已有哪些 fact、observation/lead 输入、official gap、unknown/backlog、next research target、source target 状态和禁止写入策略。
- `execution_queue.items[]` 只读表达下一步该 `run_source_target`、`repair_source_target`、`review_intelligence_context` 还是 `keep_unknown_open`，不代表 worker 已执行，也不授权写 fact edge、提升 evidence level 或关闭 unknown。
- `execution_queue.items[].source_target_actions[]` 必须直接携带 `check_target_id`、`source_adapter_id`、`target_kind`、`state`、`failure_kind`、`latest_event_type`、`recommended_cli_command`、`writes_truth_store` 和 `requires_database`。消费层不能解析 Markdown、拆字符串 refs 或重新实现 source monitor 状态规则来判断该跑还是该修。
...
`scope_kind='edge'` 可用于 fact edge 的显式未知边界，例如“该 L4/L5 single-source edge 尚未完成独立官方二源处置”。`research-pack` 的 `official-disclosure-readiness` 只生成确定性的 `proposed_unknown` payload；它不是事实证据，也不是已落库 unknown。只有 `@supplystrata/evidence-maintenance` 的受控 materialization 用例会默认检查目标 edge 仍为 `validity='current'` 后写入 `unknown_items`，并通过 unknown repository 记录 `UNKNOWN_ADDED/UPDATED` semantic change；这条路径不能写 `edges`。同一个 feature 也会消费 `OFFICIAL_DISCLOSURE_SIGNAL_DISPOSITION_RECORDED` 中 decision 为 `record_single_source_unknown` 的审计结论，把人工复核后的 single-source 缺口物化为 deterministic edge unknown；其它 disposition 只保留为 review 上下文，不会落 unknown。`WorkbenchModel.unknown_items` 必须导出 `scope_kind / scope_id`，readiness 优先按结构化 scope 识别 edge unknown，文本匹配只用于兼容旧快照。
...
当前 `kind` 包括 `supplier_list_row`、`entity_source_candidate`、`semantic_change`、`osh_facility_candidate`、`claim_conflict_review` 和 `official_disclosure_signal`。`official_disclosure_signal` 来自官方 IR / 披露文本中的确定性信号规则，只说明某份官方文档出现了供应链、产能、需求或技术路线相关内容；它保留 `fact_write_policy.automatic_fact_mutation_allowed=false`，approved 后也只写审计事件，不写 `edges`。research-pack 的 `official_disclosure_signal_correlation_hints` 只是把这些 signal 和当前 `corroboration_queue` 做只读排序提示，输出 match reason 和 `review_only_no_fact_mutation` policy；它不是事实表，不会写回 `review_candidates`，也不能被当作二源证据。人工处理这类 hint 时，`review-store` 会把 disposition 记录为 review-scoped `change_records.change_type='OFFICIAL_DISCLOSURE_SIGNAL_DISPOSITION_RECORDED'`，decision 只能是 `supports_existing_edge`、`needs_more_evidence`、`not_relevant`、`record_single_source_unknown` 或 `create_counterparty_source_target`，并且必须保留 `automatic_fact_mutation_allowed=false / allowed_edge_mutation='none'`。`record_single_source_unknown` 只是授权后续受控 unknown materialization；`evidence-maintenance` 读取该审计事件后只写 edge-scoped `unknown_items`，仍不能写 `edges` 或 `evidence`。`claim_conflict_review` 来自 `conflict_review` safe-write packet：它必须保留 `safe_write_status='blocked_pending_review'` 和 `fact_write_policy.automatic_fact_mutation_allowed=false`，只表示“该 claim 需要人工处理”，不能作为自动 deprecate edge 或自动改 claim status 的授权。approved 后执行 `review apply` 只会写 `CLAIM_CONFLICT_REVIEW_APPLIED` 和 `REVIEW_APPLIED` 审计事件；事实边、claim status、unknown status 仍保持原样。更细的人工 resolution action 通过 `CLAIM_CONFLICT_RESOLUTION_ACTION_RECORDED` 写入 `change_records`：`confirm_claim_valid` 会在 resolution evidence 存在时关闭 linked unknown，`recommend_edge_deprecation` 只记录建议，`request_more_evidence` 只记录继续调查要求；这些 action 仍不修改 `edges` 或 claim status。
...
- `@supplystrata/evidence-maintenance` 也提供 single-source disposition unknown materialization：它消费 readiness 输出里的 `proposed_unknown`，也能消费 official signal disposition 中的 `record_single_source_unknown` 审计事件；两条入口都只写 `unknown_items`，用于把二源处置缺口变成可审计 backlog；它不会把 single-source silence 或 review-only signal 解释成 corroboration。
...
- `investigation-backlog.json/md`：把 readiness gap、explicit unknown、组件覆盖缺口、corroboration queue item 和 source-plan item 转成下一步调查任务。其中 `corroboration_review` 是逐 edge 的二源检查任务，会引用 edge、candidate official source、runnable target、source target coverage 和 preflight 状态；summary 会统计 corroboration reviews 的 runnable target、coverage、disposition-only、need sync、need enable、due、failed preflight、missing credentials、invalid config、unsupported connector 和 source unreachable 数量。它只指导 review/sync/run，不写事实边。
- `corroboration-source-plan.json/md`：从 `investigation-backlog` 的 `corroboration_review` 中抽取 runnable target，按 source/target/config 去重后输出标准 `source_plan` 子集，并附带 edge、unknown、coverage 和 preflight refs。它可以直接作为 `sources policy preview-plan-targets / smoke-plan-targets / sync-plan-targets / enable-plan-targets` 的 `--source-plan` 输入，用于把二源检查接入统一 source monitor 配置链路；它不抓源、不写 observation、不写 fact edge，也不代表二源 corroboration 已经成立。
- `source-target-coverage.json/md`：把 runnable source-plan target 与 `source_check_targets / source_check_jobs / source_change_events / observations` 对齐，展示数据准备链路的 sync、enable、due、job、event、degraded 和 observation 状态。若同一 `target_config` 已在其它 namespace 跑过，coverage 可以复用该运行态，但 DTO/Markdown 必须同时暴露 `expected_check_target_id`、`matched_check_target_id` 和 `match_kind`，避免递归 research pack 把跨包复用误读为当前 namespace 已创建目标。summary 会额外输出 total observations、observed subject entities、observations by source、observations by target kind 和 observations by metric，用来衡量官方源观测深度；这些统计仍只是数据准备质量，不代表事实边成立。metric 聚合会生成只读 `observation_review` seeds，把 `purchase_obligations` 标为 P0 供应链信号，把 inventory/capex/cost/accounts payable/trade/price 类指标标为 P1，把 revenue 标为 P2 financial context，未知指标进入 mapping gap；每个 seed 都带 `review_policy='review_only_no_fact_mutation'`、source target refs 和最多 10 条 `sample_observations`。报告还会从样本生成只读 `calibration_candidates`，推荐标签限定为 `useful_signal / background_context / needs_context / not_useful`，供前端/host app 审查和 gold label 抽样；truth-store 模式会把已落库的 `observation_calibration_labels` 回灌到 candidate 的 `existing_labels / latest_label / review_status`，并在 summary / manifest 中区分 recommended label、persisted label、labeled/unlabeled 候选数。`labeling_plan` 会用 `stratified_unlabeled_by_priority_metric` 从未标注候选中确定性抽取下一批样本，按 priority 和 metric 分层，避免 gold label 集只覆盖单一指标；它只是审查队列建议，不自动写 label。它不自动修改事实边、不自动改 observation、不自动改 evidence level。`latest_job.failure_kind` 是运行态排障分类，当前包括 `missing_credentials`、`target_config_invalid`、`source_unreachable`、`source_response_error`、`rate_limited`、`adapter_error` 和 `unknown_failure`；它只来自 source-check job 的错误上下文，用于调度和排查，不是 evidence、fact edge 或风险结论。
- `source-target-preflight.json/md`：可选；打包显式传入的无数据库 source-plan smoke 结果，展示 `plan / fetch / normalize` 连通性、失败和 degraded fallback，并按 source 输出 checked/failed/skipped、normalized/degraded、target kind 和 issue kind 分布矩阵。normalized document 还会带 `observation_drafts`、`semantic_sections`、`observation_types` 和 `semantic_section_kinds`，表示现有规则能识别出的只读抽取潜力；它们不是已持久化 observation，也不能作为 fact edge 或 corroboration 结论。issue kind 当前包括 `missing_credentials`、`target_config_invalid`、`connector_unsupported`、`source_unreachable`、`source_response_error` 和 `adapter_error`。当 issue kind 是 `missing_credentials` 且 connector capability 提供了 credential contract 时，item 会携带结构化 `missing_credentials[]` env key，而不是只把 key 藏在错误字符串里。真实 source key 统一由 `@supplystrata/config` 读取，默认本地路径是 git 忽略的 `config/source-credentials.local.json`，`.env` / 环境变量可覆盖；未配置 key 只影响对应 target 的 preflight / run-due 状态，不阻断研究包生成。它不是事实覆盖，也不代表 target 已同步；如果同时传入 `investigation-backlog`，backlog 会把 failed/skipped/degraded preflight 作为同步前行动前置条件。
...
- `official-disclosure-readiness.json/md`：汇总 Gate 1 相关的研究 target profile、Gate 1 scorecard、逐节点覆盖矩阵、显式 target node 覆盖、逐 expected source 覆盖矩阵、edge-level corroboration queue、official disclosure signal correlation hints、profile expansion candidates、Level 4/5 fact edge 数、完整 traceability、严格 cross-source corroboration、strength/freshness 覆盖、explicit unknown，以及官方披露 source-plan target 的 sync / enable / due / degraded / observation 状态。scorecard 会拆分 `overall_progress / data_progress / source_path_progress`；data progress 不会把 source-plan 或 connector 当作事实边，source path progress 只说明官方源准备路径是否可执行。corroboration queue 只把 single-source / missing-evidence 的 L4/L5 edge 逐条排进待查清单，标出已有来源、候选 counterparty/source target、linked unknown、是否已有 single-source disposition unknown，以及缺失时的确定性 `proposed_unknown` payload；official disclosure signal correlation hints 只把 review-only signal 和这些待查边做可解释排序提示，并按 disposition change 区分 `open` / `recorded`，不会把沉默或 signal 自动算成 corroboration，也不会把 proposed unknown 自动写入 `unknown_items`。`corroboration-source-plan` 会给每个 filtered target 输出 `next_action / next_action_reason`，把 preflight 和 source-target coverage 翻译成配置凭据、修 target、smoke、sync、enable、run due、等待 job、排查失败或 review observation 的确定性下一步；已有 `succeeded` source target 会直接进入 `review_observations`，不会因为缺少 preflight 快照而重复提示 smoke。
- `supply-chain-expansion-plan.json/md`：把当前 research pack 的 L4/L5 fact frontier 转成递归展开计划。每个 frontier item 必须带 `edge_id / path_depth / expansion_state / component_id / unknown_ids / source_plan_refs / action`；每个 component dependency lead 必须来自 `component-context` taxonomy，并标明 `fact_covered / source_path_runnable / source_path_planned / observation_layer_only / lead_only`、`source_path_authority`、source relation policy 和 output layer。source path 状态只能来自具体 dependency、target，或锚定到 target component 的 source-plan item；parent component 的泛化 source-plan 不能借给所有下游 lead。该文件只做 planning，不写事实层；`needs_component_context`、`stop_depth_limit`、`catalog_boundary` 和 `observation_layer_boundary` 都必须显式输出，防止把公司最大供应商循环误用成无约束事实推断。
- `gate1-data-depth-workbench.json/md`：只读合并 official readiness、source target coverage、supply-chain expansion plan 和 propagation readiness，把 L4/L5 增长、二源 corroboration、source blocker、strength 缺口、observation labeling batch 和 propagation context 缺口排成 `review_only_no_fact_mutation` 优先级清单。每个 item 带 `frontend_action_kind`、`recommended_decision`、`allowed_decisions`、`write_impact` 和 `command_hints[]`，让前端/host app 可以受控展示同步、运行、标注、disposition 或递归研究动作；真实失败、retry_wait、degraded 或 dead 的 source blocker 会带具体 source target refs，排障 / rerun 命令必须使用精确 `--check-target-id`，不能退回 `--source` 粗过滤。propagation item 还会透传 `readiness_answers`，把每层 facts、non-fact inputs、official gaps、unknown/backlog、next research、source target state refs 和 output policy 前置给未来 AI/前端，并透传 `execution_queue`，把 `run_source_target / repair_source_target / review_intelligence_context / keep_unknown_open` 拆成带 refs、priority、repair reason 和 no-fact-mutation policy 的结构化队列。非空时还会拆出 `gate1-data-depth-p0.json`、`gate1-data-depth-source-blockers.json`、`gate1-data-depth-labeling.json`、`gate1-data-depth-corroboration.json` 和 `gate1-data-depth-intelligence-context.json` action batch。它们不能被当作 evidence，也不是自动写入授权。
- `gate1-run-ledger.json/md`：主执行账本会把 propagation execution queue 汇总到 `propagation_execution`，并把每层 run source target、repair source target、review intelligence context、keep unknown open 合并进 `action_queue`。run source target 会按状态拆成 `sync_targets`、`run_due_targets`、`wait_for_scheduled_targets` 或 `review_observations`：`scheduled` 只给等待/预览动作，不进入可批准运行队列；repair source target 会进入 `investigate_source_failures`，避免消费层把 not_synced、due、scheduled、succeeded、missing credentials 和 degraded target 混成同一种动作。它只给前端/host app 一个统一执行面：source target 可以进入 monitor sync/due/wait/triage，context 可以进入 review，unknown 可以保持显式边界；它不重新计算 readiness，不解析 Markdown，也不授权自动写 `edges / evidence / unknown_items`。
- `research_lineage`：递归 research pack 必须把父公司、父组件、seed edge 和 seed unknown 写入 manifest，并在 README 中摘要显示。该字段只解释“为什么打开这个子研究”，不能把父包的事实、evidence 或 unknown 自动继承成子包事实。

这些产物不对应事实表，不写 `edges`，也不改变 `risk_views / observations / unknown_items`。它们的作用是让人工或后续安全 agent 知道下一步该查什么、为什么查、关联哪些 source / unknown / component / edge，以及已经同步/运行/失败/退化在哪里。`investigation-backlog` 会消费 coverage 状态，把 action 从笼统的“去跑 source”细化为“先同步 target / 启用 target / 跑 due target / 等待 active job / 排查 failed/degraded job / review observation”；如果显式传入 source target preflight，backlog 会在同一 coverage 行展示 `preflight` 状态，并把 failed/skipped/degraded preflight 放在“同步/启用”之前处理。它也会消费 `observation-coverage`，把 sparse observation series 转成继续积累同序列窗口点或寻找 explicit baseline 的调查任务；还会消费 `official-disclosure-readiness`，把核心节点覆盖、Level 4/5 边覆盖、expected source 缺口、traceability、corroboration queue、profile expansion candidates 和 intelligence context gap 变成调查任务，并优先引用已有 runnable official disclosure targets。逐节点覆盖状态只用于计划：`covered_fact` 表示节点已有 Level 4/5 fact edge，`official_target_synced` / `official_target_runnable` 表示已有与该节点相关的可执行官方源路径，不能把同一个聚合 source-plan item 中其它节点的 target 借给当前节点；`official_source_planned` 表示还停在计划层，`missing` 表示当前 pack 没有官方披露覆盖入口。逐 expected source 覆盖状态更细：`connector_available` 表示后端已有该官方源 connector 但当前 profile 节点还没有具体 source-plan/target，`source_registered_unimplemented` 表示来源已在 registry 但还没有可运行 connector 或人工 review workflow，`missing_source_mapping` 表示 profile 期待的来源还没有映射到 source registry。research-pack 内置 `ai-compute-memory.v0` 目标 profile；当选中公司或组件落在该 profile 范围内时会自动使用它，无需用户手写清单，也可通过 CLI/host app 显式设为 `none`。profile 是验收锚点，不是全球供应链全集；其中 manufacturing-services 的 Apple Supplier List FY2022 target 是 review-only 官方名单入口，只把 PDF 行转成 review candidate、facility lead 和 OSH 交叉检查 target，人工确认前不能写事实边；silicon wafer / ABF substrate 的 EDINET target 是日本官方披露目录监控入口，只抓 daily filings metadata，不下载或解析正文；Foxconn / Quanta 的 TWSE MOPS target 是台湾官方披露电子文件目录监控入口，只抓目录 metadata，不下载 PDF 或写事实边。不在 profile 中但已被 fact edge、official source plan 或 runnable target 发现的节点会进入 `profile_expansion_candidates` 和 `profile_expansion` backlog，等待人工或后续安全 AI 审阅是否纳入 profile。调用方也可以向 `official-disclosure-readiness` 传入显式 target node set；传入后 Gate 1 的 core node 口径按这批目标节点的覆盖数衡量，未出现在当前 Workbench 里的核心节点也会以 `missing` 出现在矩阵中。未传 target node set 且未命中内置 profile 时，报告只能作为当前 pack 可见节点仪表盘，不能声明内置 profile 的核心研究节点已经达标。`observation-coverage` 只能从结构化 DTO 汇总类型覆盖和 series readiness，不能靠 Markdown label 猜 observation type；`official-disclosure-readiness` 只能从 Workbench edge/evidence/intelligence/source-plan/source-target-coverage DTO 计算，不能把单源沉默自动解释成已审计 single-source。`proposed_unknown` 是 review/agent 后续写入的建议 payload，不是已持久化 unknown。正式系统需要持久化时，应另建 agent/review 层契约，不能把 backlog item、preflight item、corroboration queue item、proposed unknown 或 coverage item 当成事实证据。

当前 `ai-compute-memory.v0` profile 已把 AI server、PCB 上游和工艺材料 frontier 纳入验收锚点：server 侧包含 GPU、HBM、manufacturing services、PCB、optical module、power supply 和 cooling；PCB 侧继续展开到 CCL、copper foil、electronic glass cloth 和 laminate resin；wafer/fab 侧保留 cleanroom construction context；process input 侧显式登记 photoresist、sputtering target、CMP consumables 和 specialty gases。这些节点只进入 taxonomy、source-plan、unknown、coverage 和 investigation backlog，帮助后续查源、跑监控和做人工/安全 AI 研究；它们不是已验证公司级供应关系，也不能自动生成事实边。

`source-plan` 只有在调用方显式提供足够参数时才生成 runnable target：例如 `tradeObservationMonth` 生成 Census Trade target，`commodityObservationMonth` 生成 World Bank commodity target，target profile 中带 SEC CIK 的公司生成 `sec-edgar/sec-company-filings` 和 observation-only `sec-edgar/sec-company-facts` target，`officialDisclosureYear` 生成 TSMC / Samsung / SK hynix / Micron / ASML 的 node-specific `official-html-disclosure` target；`company-ir` 还必须在 target profile/review 配置中带显式 HTTPS `url`。SEC CIK 这类显式 target config 不需要披露年份；官方 IR 这类按年度页面抓取的 source 必须有年份；`company-ir` 的长尾入口必须有 URL。缺少时间参数、CIK、URL、target config 或 connector 时保持 gap，不能猜默认 period，也不能把 registry 里登记过的来源伪装成可运行监控。`source-management` 会把 runnable suggestions 转换为 `source_check_targets`：`check_target_id` 由 namespace、source、target kind 和 target_config 稳定生成，默认 `enabled=false`。source-check connector capability 是 target 配置和 credential requirement 的统一契约；catalog、无数据库预览和无数据库 smoke 都读取同一份 `config_schema / credential_requirements`，不能在 CLI 或 research-pack 里维护另一套 env key 清单。凭据 key 的定义集中在 `@supplystrata/config`，真实值集中到 `config/source-credentials.local.json` 或环境变量，避免每个 adapter / CLI 分支各自维护 key 清单。无数据库预览会复用同一转换和 validation，输出 target id、去重统计、source / target kind / priority 汇总和 credentials warning，方便同步前审计。无数据库 smoke 会复用同一批 target config 解析和 adapter，执行 `plan / fetch / normalize` 并输出 source 连通性、degraded fallback 和 normalize 结果；需要凭据的 target 会在访问外部源前按统一 credential contract 归类缺口。它不写任何表，也不把成功抓取解释成事实覆盖。审计后可用同一 `source-plan.json + namespace` 受控启用已同步 target，并写入 target 级 cadence、jitter、retry 和 `next_check_at` 覆盖值；启用后才由 `sources due/run-due` 或 worker 执行。该转换、预览、smoke 和启用流程都不写事实边，只把研究计划接到统一监控配置层。
...
`observation_calibration_labels` 是观测层 gold label 样本池。它消费 `source-target-coverage.observation_review.calibration_candidates` 或前端/host app 选中的 observation 样本，只记录“这个观测对供应链方法学是否有用”的审查结果；truth-store research pack 会按 observation id 读取这些 label 并回填到 candidate 的 `existing_labels / latest_label / review_status`，用于显示 gold set 进度。它不写 `edges`，不修改 `observations`，也不把 observation 升级成 fact edge。
...
observation_calibration_labels
  ...
  observation_id
  candidate_id
  label: useful_signal | background_context | needs_context | not_useful
  reviewer
  reviewed_at
  rationale
  attrs
...
当前第一版由 `recordObservationCalibrationLabel()` 提供：

- `useful_signal` 表示该 observation 可作为供应链语义、异常、传播或 component context 的校准样本，但仍不能直接写事实边。
- `background_context` 表示该 observation 适合做规模化、归一化或财务背景。
- `needs_context` 表示 metric 需要先补 methodology mapping、component/geography scope 或 source 解释。
