...
- `gate1-data-depth-workbench.json/md`：只读合并 official readiness、source target coverage、supply-chain expansion plan 和 propagation readiness，把 L4/L5 增长、二源 corroboration、source blocker、strength 缺口、observation labeling batch 和 propagation context 缺口排成 `review_only_no_fact_mutation` 优先级清单。非空时还会拆出 `gate1-data-depth-p0.json`、`gate1-data-depth-source-blockers.json`、`gate1-data-depth-labeling.json`、`gate1-data-depth-corroboration.json` 和 `gate1-data-depth-intelligence-context.json` action batch，方便前端/host app 分别展示审查和配置动作。它们不能被当作 evidence，也不是自动写入授权。
