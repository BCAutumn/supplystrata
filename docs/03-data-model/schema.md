...
当前 `ai-compute-memory.v0` profile 已把 AI server 与 PCB 上游 frontier 纳入验收锚点：server 侧包含 GPU、HBM、manufacturing services、PCB、optical module、power supply 和 cooling；PCB 侧继续展开到 CCL、copper foil、electronic glass cloth 和 laminate resin；wafer/fab 侧保留 cleanroom construction context。这些节点只进入 taxonomy、source-plan、unknown、coverage 和 investigation backlog，帮助后续查源、跑监控和做人工/安全 AI 研究；它们不是已验证公司级供应关系，也不能自动生成事实边。
