...
```
1. 看 source authority matrix：先得 source_cap
   regulator + self_disclosure（SEC 10-K / 10-Q / 20-F / 8-K）→ 5
...
2. 看 relation_authority + relation_type：再得 relation_cap
   ...
   facility_claim → 受 source_cap 限制，通常最高 4
   registry_fact → 对 OWNS_SUBSIDIARY / OWNS_BUSINESS_UNIT / OPERATES_FACILITY 可到 4；对 BUYS_FROM / SUPPLIES_TO 只能到 2
   macro_trend → 只能到 2，进入 observations / context，不直接证明公司关系
   lead_only → 只能到 1，进入 hypothesis / review
...
自动 scorer 当前先落地 `source_cap / relation_cap / method_cap / modal_cap` 的硬上限。跨证据 promotion 由 graph-builder 或后续 claim layer 负责，不能在单条 evidence 打分时偷偷升等级。
