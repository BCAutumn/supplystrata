...
当前方法学目标是把 AI compute 主链的每一层变成可审计的研究输入，而不是让规则或 AI 直接产出产业结论。也就是说，系统应先准备 `facts + observations + leads + unknowns + source targets + calibration labels + execution queue`，再交给前端研究流程或安全 AI 做综合分析。任何自动流程都不能因为某条行业叙事“看起来合理”就写 fact edge、提高 `evidence_level` 或关闭 unknown。
...
matrix layer 和 Gate 1 propagation action 还必须输出 `readiness_answers`。它不是新证据，也不是结论层；它只是把每层必须回答的七个问题收口成稳定 DTO：L4/L5 fact refs、observation/lead 输入、official evidence gap、explicit unknown/backlog、next research target、source target 状态，以及允许输出/禁止 truth-store 写入。source target answer 必须同时给出 summary 和具体 `runnable_refs / blocked_refs / degraded_refs / missing_credentials_refs / source_failed_refs`，让未来前端或安全 AI 能直接知道“该跑哪批 target、该补哪个 key、该排查哪个 source”，而不是解析 Markdown 或重新实现状态规则。未来前端或安全 AI 应优先消费这个字段来回答“已有事实是什么、哪些只是观测/线索、缺什么、下一步查什么、现在能不能跑源、哪些写入绝对禁止”，不能再从 Markdown 或 refs 字符串里二次拼装权限边界。

matrix layer 和 Gate 1 propagation action 还必须输出结构化 `execution_queue`。它把当前层下一步拆成 `run_source_target`、`repair_source_target`、`review_intelligence_context`、`keep_unknown_open` 四类可消费队列项，并带 priority、source target refs、official gap refs、unknown refs、next research refs、repair reason 和 `automatic_fact_mutation_allowed=false`。其中 `run_source_target` 和 `repair_source_target` 必须继续带 `source_target_actions[]`，逐 target 暴露 `check_target_id`、source adapter、target kind、state、failure kind、latest event、推荐 CLI 命令、是否写 truth-store 和是否需要数据库。主执行账本把这些队列项转成可执行 action 时，必须继续按 source adapter、target kind 和固定小批次收窄命令，并使用精确 `check_target_id`；不能因为同一个 source adapter 里还有其它 target 就扩大执行范围。这不是 worker 指令，也不是写库授权；它只是让前端/host app/未来安全 AI 不需要解析 Markdown、拆 refs 或重算 source monitor，就能知道这层该跑、该修、该审，还是继续显式保留 unknown。
...
- `gate1-data-depth-workbench` 在 readiness 和 run ledger 之上输出：它把 L4/L5 fact edge gap、counterparty corroboration queue、source blocker、edge strength gap、observation calibration labeling batch 和 propagation context 缺口合成 `review_only_no_fact_mutation` 优先级清单。每个 item 都显式列出 frontend action kind、推荐决策、允许决策、写入影响和命令提示；真实运行失败 / degraded / retry_wait 的 source blocker 必须落到具体 `check_target_id`，命令提示使用精确 `--check-target-id`，不能要求消费层按 `--source` 粗过滤后自行猜目标。这些字段只帮助下一轮跑数、排障和 gold label 扩样，不写 `edges / evidence / unknown_items / observations`，也不把 observation 或 official signal 升级成事实证据。
...
[x] AI compute propagation matrix 和 Gate 1 propagation action 能输出 `execution_queue`，把 run / repair / review / keep unknown open 的下一步动作结构化给未来前端和安全 AI，且不授权自动写 fact edge、提升 evidence level 或关闭 unknown
...
说明：ComponentCard 已能展示 component risk baseline、component-scoped observation anomaly summary，以及由当前组件事实边关联到 supplier/consumer 的 company financial signals；CompanyCard 已能展示 company-scoped observations、observation anomaly summary，并基于 component risk metrics 聚合出 top exposure nodes。research-pack 默认只读打包；需要刷新当前包内 eligible component risk baseline 时必须显式传 `--prepare-data` 或 `--refresh-component-risk`，且 eligible 的前提是已有可审计 Level 4/5 component fact edge。component risk baseline 的计算范围是 `component_global`，不是当前公司子包的事实边集合；research-pack manifest 必须同时输出 `component_risk_global_edges` 和 `component_risk_visible_edges`，让审计者区分“组件全局风险上下文”和“当前 selected company / depth 可见的 fact edge”。只有 taxonomy、source-plan 或 observation 的组件必须继续保留为 coverage gap，不能写空风险结论。`official-disclosure-readiness` 是 Gate 1 的数据账本：它只读取 Workbench 里已有事实边和 evidence，统计研究 target profile、逐节点覆盖状态、显式 target node 覆盖、逐 expected source 覆盖、edge-level corroboration queue、profile expansion candidates、Level 4/5 fact edge、完整 traceability、严格 cross-source corroboration、strength/freshness 覆盖和 explicit unknown；同时读取 official source-plan / source-target coverage，把 runnable target 的 not_synced、disabled、due、active、degraded、dead 和 observation 状态带进报告和 backlog action。`ai-compute-memory.v0` 是内置确定性研究验收锚点，不是全球供应链全集；系统会按选中公司/组件自动选择该 profile，也允许调用方显式关闭或覆盖 target nodes。内置 profile 会把已有 SEC CIK 作为 source-plan hint 下沉给 `sec-edgar/sec-company-filings`，不依赖官方披露年份；给定 `officialDisclosureYear` 时，TSMC / Samsung / SK hynix / Micron / ASML 的官方 IR source 会生成 node-specific runnable target，显式配置了 HTTPS URL 的 `company-ir` 目标也会生成 runnable target；Samsung / SK Hynix 还会生成 `dart-kr/company-filings` runnable target，silicon wafer / ABF substrate 会生成 `edinet/daily-filings` runnable target，Foxconn / Quanta 会生成 `twse-mops/electronic-documents` runnable target，manufacturing-services 目标会生成 Apple Supplier List FY2022 的 `apple-suppliers/supplier-list-review` target。profile 的 process input frontier 也已经登记为可校验组件节点：photoresist、sputtering target、CMP consumables 和 specialty gases 会进入 source coverage、unknown 和 backlog 口径，但不会自动写事实边。缺少显式 URL 的 `company-ir` 节点和缺少公司级 EDINET code 的更细目标仍显示为 connector-only 或配置缺口，不会被自动校准成已覆盖。不在 profile 中但已经通过事实边、官方 source-plan 或 runnable target 出现的节点会进入 expansion candidates/backlog，等待人工或后续安全 AI 审阅是否纳入 profile。调用方传入 target node set 后，Gate 1 core node 口径按这批目标节点中非 `missing` 的数量衡量；未传 target node set 且未命中内置 profile 时只能回退到当前 pack 可见节点，不能宣称核心研究节点已完成。它不会把 single-source silence 当作已解释，也不会生成事实边。`investigation-backlog`、`corroboration-source-plan`、source target smoke / sync / enable / run-due、source credentials 和 source-target coverage 仍然只负责把缺口变成受控行动；它们不抓取无关源、不把二源候选升级成 fact edge、不把 observation/lead 伪装成事实。当前 anomaly、component risk、weighted impact、path redundancy、weighted centrality 和 alert policy 已有 deterministic baseline；真实世界样本校准、阈值治理和误报治理仍未完成。
