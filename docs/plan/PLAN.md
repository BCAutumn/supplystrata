...
- **B2 SDK 版本兼容**：npm registry 上的官方包名是 `@modelcontextprotocol/sdk`，当前锁定 `1.29.0`；API 可能继续演进。当前 SDK 的 `ToolAnnotations` 类型不直接包含 `requires_user_confirmation`（这是较新的 MCP spec 字段），B4 需要决定如何处理并加 contract test。**继续 B4 前需要一起决定**：(a) PR 给 SDK；(b) 私有 wrapper / contract 层注入；(c) 等 SDK 升级。
