...
| A5  | Implement helper schemas + cite validation                         | A1 只是签名；A5 实现 disambiguate_entity / derive_dynamic_profile / suggest_source_targets / summarize_with_citations 的 provider 调用、prompt template、输入 schema validation、输出 schema validation、cite 校验和 ≥ 2 unit test                                                                                  | 4 个 helper 每个 ≥ 2 测试；fixture 覆盖：正常 / 模糊 / 无效输出 / disabled mode                                                              | 没有 helper 直接返回 LLM 原始文本；都经过 schema + cite 校验；helper README 写清每个输出的"不允许被当成什么用"                                                         |
...
- [x] `packages/ai-analysis/src/` 文件数 ≤ 6（删 `local-simulated-analysis.ts` + 两个 `provider-*.ts`）
- [x] `rg "fetch.*api.openai|fetch.*deepseek" packages/` 只命中 `packages/llm-helpers/`
- [x] `rg "from \"@supplystrata/ai-analysis\"" apps/ packages/` 不再命中 `provider-*` / `local-simulated-*` 导出
- [x] `pnpm dep-check` 通过；故意构造违例验证拦截有效
- [x] `packages/llm-helpers/README.md` 完整（4 个 helper 用法 / disabled 行为 / provider 配置 / 与 ai-analysis 关系）
- [x] `packages/agent/README.md` 写清"独立 optional、不被核心依赖、可整包删除"
- [x] 无 `// TODO` / `// FIXME` / `// HACK` 残留
- [x] 无 `deprecated` / `legacy` / `_old` / `.bak` 文件
- [x] `pnpm type-check` / `lint` / `test:unit` / `dep-check` / `format:check` 全绿
- [x] `pnpm smoke:local` / `smoke:research` 行为不变
- [x] `tests/e2e/nvidia-fixture.test.ts` 通过
- [x] `docs/02-architecture/module-design.md` 中 `packages/llm-helpers/` 和 `packages/agent/` 的【新增，目标】标签去掉
...
- Phase A 已完成：`@supplystrata/llm-helpers` 成为唯一 LLM helper/provider 边界，4 个 helper 均为 candidate-only，并带输入校验、输出校验、citation 校验、disabled/deferred/invalid/provider_error 状态。
- `@supplystrata/agent` 已作为 optional reference package 占位落地；dep-check 禁止核心 packages 依赖 agent。
- dep-check 已机械化拦截：fact/derived 写入路径不得 import `llm-helpers`，`llm-helpers` 不得反向依赖 `ai-analysis` domain，`packages/*` 不得依赖 reference agent。
- `research ai-analyze` 继续由 `ai-analysis` 负责 artifact 校验和 audit ledger 写入；LLM/provider/local candidate 构造在 `llm-helpers`，且 `llm-helpers` 不再依赖 `@supplystrata/ai-analysis`。
- 收尾验证：`pnpm type-check`、`pnpm lint`、`pnpm test:unit`、`pnpm dep-check`、`pnpm build`、`pnpm format:check`、`pnpm smoke:local`、`pnpm smoke:research`、`pnpm test:e2e -- nvidia-fixture` 均通过。
