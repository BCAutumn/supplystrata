...
类型边界必须按生命周期命名和放置：Domain Contract 描述业务概念，Persistence Row 只描述 SQL 查询结果，Workbench/API DTO 只描述对外消费契约。`source-monitor`、`workbench-export` 和 `card-builder` 的 `db-rows` 是这条规则的样例：Row 形状不再混在业务输入类型或导出编排里，内部查询直接依赖 `db-rows`，对外只暴露稳定 DTO / model。后续治理 `core`、`db`、`chain-view-builder` 时也按这条线推进，禁止为了省事把 DB Row 当 DTO 或把 DTO 字段塞回 domain contract。
