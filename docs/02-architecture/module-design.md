...
│   ├── object-store/                       # 抽象的对象存储（本地FS / MinIO）；存储根路径由 app/CLI 显式注入
...
| `chain-view` / `chain-view-builder`                                  | 暂不合并                                                                                            | `chain-view` 必须保持纯 DTO，builder 可依赖 DB；builder 内部 SQL Row 与 segment 映射必须分离，这是有意防腐 |
...
graph  ← CLI graph 命令消费；当前内置 Neo4j adapter，连接参数由 app/CLI 显式注入
...
card-builder ← apps/cli / 后续 API 消费；负责从 DbClient 组装 CompanyCard / ComponentCard / ChainCard / EvidenceCard / UnknownMap DTO；SQL row 形状集中在 `db-rows`，row→card DTO 纯映射集中在 `dto-mappers`
chain-view-builder ← card-builder / CLI / 后续 API 消费；负责从 DbClient 组装 ChainViewModel，允许查库和拼接 context segment；SQL row 形状集中在 `db-rows`，row→segment model 的纯映射集中在 `segment-mappers`，`index` 只保留 query/use-case 编排和 source hint 计划调用；不得把 DB Row 暴露成 `chain-view` DTO，也不得在 segment mapper 里读库或写库
workbench-export ← CLI / research-pack / 后续 host app 消费；组装稳定 Workbench JSON 契约，包含事实边、claim、evidence、unknown、source health、changes、attention queue、review queue 摘要和派生 intelligence context；稳定 DTO 契约集中在 `definitions`，Workbench 内部 SQL row 形状集中在 `db-rows`，DTO 映射集中在 `dto-mappers` / `claim-dto`，读取兼容和 schema 校验集中在 `schema`；`review-queue` 只读，会带出 official signal disposition 审计结果，但不修改 review 状态、不写事实边；attention queue 组装保持在独立 feature 文件，claim conflict context 通过 `claim-builder` public contract 获取，避免导出层复制 claim 裁决规则
research-pack ← CLI / 后续 host app 消费；编排已有 truth-store 数据，导出 workbench、cards、source plan、quality report、question/observation/official-disclosure readiness、investigation backlog 和 filtered corroboration source plan，不抓新源、不写事实边；稳定 pack DTO 契约集中在 `definitions`，显式写库准备动作集中在 `prepare-data`，manifest/stats 组装集中在 `manifest`，文件写出、README 和 Markdown 索引集中在 `writer`，`index` 只保留 build use-case 编排；investigation backlog 的公共契约、Markdown 渲染、source-target 覆盖上下文分别放在 `investigation-backlog-definitions`、`investigation-backlog-render`、`investigation-backlog-source-targets`，主入口只保留 backlog 生成规则和原有 public export；official-disclosure readiness 可接收显式 target node set，也会自动选择内置 `ai-compute-memory.v0` 研究 target profile，让 Gate 1 核心节点覆盖按目标清单衡量，而不是按当前 pack 可见节点猜测；逐 expected source 覆盖会把 profile 期待来源拆成已覆盖、已同步/可同步、仅计划、connector 可用但未接线、registry 已登记但未实现、未映射等状态；`official-disclosure-signal-correlation` 是 research-pack 内部纯算法模块，只把 review-only 官方 signal 和 edge-level corroboration queue 做确定性关联提示，不读库、不写 review、不把 signal 升级成 corroboration；official signal disposition 由 `review-store` 记录成 review-scoped change，research-pack 只读取并区分 open / recorded hints；corroboration-source-plan 只把逐 edge 二源检查 target 过滤成 source-management 可消费的标准 source-plan 子集，不运行 adapter、不生成事实；Gate 1 run ledger 的 `monitoring_config` 是面向后续前端/host app 的 source policy 配置契约，只暴露 namespace、cadence、jitter、retry、backoff、初始 `next_check_at` 和 action batch，不抓源、不写事实边；监控配置/批次判断集中在 `gate1-run-ledger-monitoring`，它只消费 source-target coverage 和 preflight 汇总，把 disabled、due、retry_wait、degraded、source_failed、observation 等运行态转成 operational action，不把这些信号升级成证据；`review_workbench` 是审查契约，只输出 review item、推荐决策、允许决策、写入影响和 `review_only_no_fact_mutation` policy，让规则先排序、人工只确认边界动作；profile expansion candidates 只进 backlog/review，不写事实边
  - official-disclosure readiness 内部按职责拆分：`official-disclosure-readiness-definitions` 只放公共契约，`official-disclosure-readiness-render` 只做 Markdown 渲染，`official-disclosure-readiness-source-context` 只构造 source plan / target coverage / expected source / corroboration queue 上下文，`official-disclosure-readiness-summary` 只计算汇总口径和 signal 摘要；主入口保留 Gate 1 report orchestration、edge traceability 与 gap 生成，避免再次长成跨职责大文件。
...
类型边界必须按生命周期命名和放置：Domain Contract 描述业务概念，Persistence Row 只描述 SQL 查询结果，Workbench/API DTO 只描述对外消费契约。`source-monitor`、`workbench-export`、`card-builder`、`chain-view-builder`、`claim-builder`、`graph-builder`、`data-quality`、`entity-import` 和 `evidence-maintenance` 的 `db-rows` 是这条规则的样例：Row 形状不再混在业务输入类型或导出编排里，内部查询直接依赖 `db-rows`，对外只暴露稳定 DTO / model。后续治理 `core`、`db` 时也按这条线推进，禁止为了省事把 DB Row 当 DTO 或把 DTO 字段塞回 domain contract。

环境变量和 logger 只能在 app/worker/CLI 或 use-case 边界解析；feature 内部应接收显式 `Env`、adapter context 或 logger port。Source adapter 不得直接读取 `.env` 或 API key；`AdapterContext` 承载 user agent、snapshot store 和 source credentials，`source-workflows` 只提供 `sourceWorkflowAdapterContextInput(env)` 这类装配函数，不能在 preview、entity lookup、review enqueue 或 source check 的深层调用里自行读取 `.env`。`source-plan-smoke` 和常规 `source-check` run input 必须包含显式 `Env`，缺凭据预检调用 `@supplystrata/config` 的 credential resolution，而不是在 smoke/check 内部读 `process.env`；source-check runtime 通过 connector run context 向 adapter check 同时传入 logger 与 adapter context input，避免深层 feature 自己创建默认 singleton 或重新读环境。`getLogger()` 仍可作为 app/worker/CLI 边界默认实现，但不能成为新业务规则内部的隐式依赖；library/use-case 默认无日志时使用 `noopLogger`，由调用方决定是否注入真实 logger。`db`、`graph` / `object-store` 这类基础设施 adapter 也不得默认读取环境变量，必须由 app/CLI/worker 显式传入 Postgres、Neo4j 连接参数或本地对象存储根路径。

`db` 的 root 包入口只保留 `createDatabaseStore`、`dbTxClientBrand` 和基础连接类型，不能再作为全量 barrel 使用。业务代码必须选择更窄的子路径：`@supplystrata/db/read` 只暴露只读 repository 和 read-side Row 类型，`@supplystrata/db/write` 暴露受控写入 repository 与事务客户端类型，`@supplystrata/db/admin` 暴露 migration / seed / store 构造入口。`DatabaseStore` 只负责生命周期、`read` 端口和 `transaction()`，不继承 `DbClient`；调用方不能再把 store 当普通 query client 传入 repository。内部按职责拆分：`client` 负责 `DatabaseStore` 接口、Postgres adapter 与 migration 调用，`seed` 负责 CSV seed 与必要的数据回填，`documents` 负责 normalized document / chunk / review queue 写入，`pending` 负责待解析实体，`query` 负责边、证据、unknown map 的只读查询。新增仓储函数必须优先落在对应职责文件中，并从 read/write/admin 中选择性导出，避免重新膨胀成单文件数据库工具箱。
...
`source-workflows` 是具体免费/公开源的 use-case 编排层：SEC EDGAR 纵向切片、Apple Supplier List review enqueue、GLEIF / Companies House / OpenCorporates entity lookup、官方 IR preview、OpenDART disclosure list monitor、EDINET daily filings monitor、Census / OSH source check connector 都放在这里。这里的文件和 adapter id 表示“来源发布方或来源类别”，不是“被研究公司”。例如 `apple-suppliers` 是 Apple 官方 Supplier List 这个特殊 PDF 来源；研究 NVIDIA、AMD、Tesla 或任意上市公司时，入口仍应是 `entity resolver -> source-plan -> SEC/监管披露/company IR discovery -> review/backlog`，不能新增 `nvidia-suppliers.ts`、`tesla-suppliers.ts` 这类公司专属工作流。GLEIF 是跨市场法人标识锚点，先作为 entity-source feature 放在 `source-workflows`，不新增薄包；DART / EDINET / TWSE / HKEX 这类监管源优先复用同一条 source-check 骨架和统一 target config 契约，而不是各自重做实体消歧或调度分支。它可以依赖 `sources/*`、`source-connectors` 和 source-monitor 窄写入口，但不能反向依赖 `pipeline` 或 graph projection 包；完整 SEC pipeline 由 CLI / app-level orchestration 显式组合 fetch + normalized document pipeline。`source-check-catalog` 是只读 source-check 能力目录，只描述每个来源的运行态 connector、smoke adapter、target config parser 和 adapter context factory；它不执行抓取、不写库、不读取环境变量。`source-check-registry` 只从 catalog 投影运行态 connector，`runDueSourceChecks()` 只负责 enqueue / claim `source_check_jobs` 并调用 registry 分发 connector，持续监控 cadence / jitter / retry/backoff 只能来自 source policy config，后续常驻 worker 也复用这条路径。source-plan smoke 只从 catalog 投影离线预检 runner，不写库；不能和运行态 registry 硬合并成会写库的上帝对象。新增 Open Supply Hub、Comtrade 等源时应扩展 `source-workflows` 或独立 feature workflow 包，并补一条 catalog entry，而不是分别维护 runtime connector 和 smoke runner 两套清单，也不是改 `pipeline`。

`source-plan` 是二/三级链路扩源的边界：它把 `component-context` 里的上游 lead 映射到 `source-registry` 里的免费/公开数据源，并标明 `edge / observation / lead / entity` 输出层与自动化策略。它也可以消费 research target profile 传入的官方源 hints，把已有 SEC CIK 或已注册官方 IR connector 转成 node-specific runnable target suggestions；没有 target config 或 connector 的来源只能停留为 coverage gap。profile 是研究目标和验收锚点，不是每家公司一套代码；如果用户输入任意上市公司，后端应优先从 entity / registry / regulator metadata 推导可运行 target，缺口进入 backlog，而不是要求用户手写 profile 或让开发者新增公司文件。它不抓取、不解析、不写 Postgres，也不允许把 Comtrade/AIS/能源/新闻这类弱源升级成事实边。`definitions` 只放 source plan DTO、target suggestion、official-disclosure target node、trade/material/official disclosure context 和内部 draft/context 契约；`index` 保留确定性规划规则、suggestion 生成和 registry 校验，避免新增来源时继续把公共 DTO 与规划算法揉在一起。
...
`source-monitor` 是 source monitoring 的 truth-store 写入与调度状态边界：`source-check-jobs` 只负责 job enqueue / lease claim / success-failure lifecycle，`source-health-registry` 只负责 registry source health upsert 和默认 policy bootstrap，`source-check-target-selection` 只负责 target/source selection 归一化，`coverage` 只读 target coverage。`index` 保留 source health 查询、policy/target 同步、target enable、document observation / failure / degraded event 写入和 next-check 推进。后续继续拆分时必须保持这些边界，不能把 connector 分发、source-management 校验或 adapter 抓取逻辑塞回 `source-monitor`。

`evidence-maintenance` 是 truth-store 维护型 use-case 层。第一版包含 evidence trace backfill、edge intelligence refresh 和 single-source disposition unknown materialization。它可以消费 `db` repository，并按方法学写入 `edge_strength_estimates`、`edge_freshness` 和 explicit unknown；`official-disclosure-readiness` 产生的 `proposed_unknown` 只有通过这里的受控用例、且默认确认目标 edge 仍为 `current` 后，才会落到 `unknown_items`。它不得写 `edges`、不得提升 `evidence_level`、不得把 LLM / observation / lead 结果包装成事实关系。CLI 只能作为薄入口调用它，`research-pack` 可以在导出前调用它让研究输出带上最新派生上下文。component risk 内部按职责拆分：`component-risk-definitions` 只放输入/输出契约和模型版本，SQL Row 形状集中在 `db-rows`，`component-risk-repository` 只读 component-scoped L4/L5 fact edge，`component-risk-metrics` 只做确定性指标算法、fingerprint 和 metric id 计算，`component-risk` 主入口只保留刷新编排和写 risk view/change record。后续校准、阈值或图算法优化必须优先扩展纯算法文件和测试，不能把 SQL 或写库流程塞回指标函数。
...
`chain-view-builder` 负责把 `DbClient` 查询结果、component upstream lead 和 source plan hint 聚合成稳定 `ChainViewModel`。它的查询 Row 只能放在 `db-rows`，segment DTO 映射只能放在 `segment-mappers`；`index` 作为 use-case 编排层可以选择查询顺序、去重和 source hint 计划，但不能承载 label 文案、confidence 映射或 row→DTO 细节。这样后续把 ChainView 接到 API 或图形工作台时，仍能复用同一个纯 `chain-view` contract，而不是绑定 Postgres Row 形状。
