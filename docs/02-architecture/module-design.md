...
环境变量和 logger 只能在 app/worker/CLI 或 use-case 边界解析；feature 内部应接收显式 `Env`、adapter context 或 logger port。Source adapter 不得直接读取 `.env` 或 API key；`AdapterContext` 承载 user agent、snapshot store 和 source credentials，`source-workflows` 负责把 app/CLI 读取到的 env 装配成 context。`source-plan-smoke` 缺凭据预检必须调用 `@supplystrata/config` 的 credential resolution，而不是直接读 `process.env`；source-check runtime 通过 connector run context 向 adapter check 传入 logger，避免深层 feature 自己创建默认 singleton。`getLogger()` 仍可作为边界默认实现，但不能成为新业务规则内部的隐式依赖。`graph` / `object-store` 这类基础设施 adapter 也不得默认读取环境变量，必须由 app/CLI 显式传入 Neo4j 连接参数或本地对象存储根路径。
