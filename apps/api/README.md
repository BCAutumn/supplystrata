...
The HTTP adapter is intentionally thin: it starts a Node server, opens the configured Postgres store, and delegates to existing public DTO builders, source workflow use-cases, source monitor use-cases, and AI analysis contracts. The database is not the company coverage boundary; it is the evidence ledger, run/status ledger, history baseline, and change-detection memory. `GET /companies/:id/supply-chain-report` is a read-through research surface: it may bootstrap a listed-company identity, enqueue source-check jobs, and by default run due source checks inline when data is missing or stale, then returns the current report context plus observable run state. The inline source checks on this read-through path only refresh observations/documents; they do not write fact edges (`read_through_research_may_network_no_fact_edge_write`). Research-run creation (`research_run_mutation_no_fact_edge_write`) only bootstraps identity and enqueues jobs. The explicit `POST /source-checks/run` route (`source_check_execution_runs_evidence_gated_promote`) is where source-check execution runs the full pipeline through evidence-gated promote, writing rule-extracted high-confidence facts as current edges (#13); it still cannot call AI providers. Review routes: `reject` is `review_queue_mutation_only_no_fact_edge_write`, while `approve` (`review_approval_applies_reviewed_fact_edges`) applies the reviewed candidate within the same confirmation boundary.
...
- `GET /changes`（可选 `?scope=<kind>:<id>`，如 `entity:ENT-ASML` / `source:sec-edgar`，按实体/边/源过滤）
- `GET /sources/health`
