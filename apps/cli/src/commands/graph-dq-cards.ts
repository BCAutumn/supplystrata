...
export function registerGraphDqAndCardCommands(program: Command): void {
  ...
  program.command("company").argument("<query>", "company name, alias, ticker, or entity id").option("--format <format>", "markdown or json", "markdown").description("render a company card").action(async (query: string, options: { format: string }) => {
    await withPool(async (pool) => {
      write(await renderCompany(pool, query, parseFormat(options.format)));
    });
  });

  program.command("chain").argument("<query>", "company name, alias, ticker, or entity id").option("--depth <count>", "upstream traversal depth, max 5", "2").option("--format <format>", "markdown or json", "markdown").description("render a chain-first upstream view for a company").action(async (query: string, options: { depth: string; format: string }) => {
    await withPool(async (pool) => {
      write(await renderChain(pool, query, { depth: parseLimit(options.depth), format: parseFormat(options.format) }));
    });
  });

  program.command("component").argument("<query>", "component id, name, or alias").option("--format <format>", "markdown or json", "markdown").description("render a component supply-chain card").action(async (query: string, options: { format: string }) => {
    await withPool(async (pool) => {
      write(await renderComponent(pool, query, parseFormat(options.format)));
    });
  });

  program.command("evidence").argument("<evidenceId>", "EV-xxx").option("--format <format>", "markdown or json", "markdown").description("render an evidence card").action(async (evidenceId: string, options: { format: string }) => {
    await withPool(async (pool) => {
      write(await renderEvidence(pool, evidenceId, parseFormat(options.format)));
    });
  });

  program.command("unknown-map").argument("<query>", "company query").option("--format <format>", "markdown or json", "markdown").description("render unknown map").action(async (query: string, options: { format: string }) => {
    await withPool(async (pool) => {
