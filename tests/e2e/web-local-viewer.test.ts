...
describe("local SCBOM viewer app", () => {
  ...
  it("serves a localhost viewer shell wired to the configured MCP endpoint", async () => {
    ...
    expect(html.indexOf("<scbom-evidence-view>")).toBeLessThan(html.indexOf("<scbom-unknown-map>"));
    expect(html.indexOf("<scbom-unknown-map>")).toBeLessThan(html.indexOf("<scbom-supply-chain-graph>"));
