...
describe("NVIDIA fixture e2e", () => {
  ...
  it("runs parser, rule extraction, scoring, graph apply, render, and unknown map without network", async () => {
    ...
    const summary = await runSupplyChainPipelineFromNormalized(pool, { normalized });
    ...
    try {
      ...
      await expect(builder.checkConsistency()).resolves.toMatchObject({ status: "synced" });
    } finally {
      await builder.close();
    }
...
async function cleanupFixtureRows(client: pg.Pool): Promise<void> {
  ...
  await client.query("DELETE FROM extraction_review_queue WHERE doc_id = 'DOC-E2E-NVIDIA-10K-FIXTURE'");
...
async function listFixtureEvidenceEdgeIds(client: pg.Pool): Promise<string[]> {
  const result = await client.query<{ edge_id: string | null } & pg.QueryResultRow>("SELECT DISTINCT edge_id FROM evidence WHERE doc_id = 'DOC-E2E-NVIDIA-10K-FIXTURE'");
...
async function listFixtureEvidenceIds(client: pg.Pool): Promise<string[]> {
  const result = await client.query<{ evidence_id: string } & pg.QueryResultRow>("SELECT evidence_id FROM evidence WHERE doc_id = 'DOC-E2E-NVIDIA-10K-FIXTURE'");
