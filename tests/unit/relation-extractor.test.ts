...
describe("SEC official supply-chain rule extractor", () => {
  ...
  it("extracts named major customer disclosures without relying on NVIDIA-specific logic", () => {
    const candidates = extractFromSentence("Sales to Microsoft accounted for 18% of our total revenue from GPU products during fiscal 2026.", "fixture", {
  ...
  it("keeps anonymous customer concentration out of company edges", () => {
    const candidates = extractFromSentence("One customer accounted for 21% of total revenue in fiscal 2026.", "fixture");

    expect(candidates).toHaveLength(0);
  });

  it("does not turn generic customer lists into SUPPLIES_TO edges", () => {
    const candidates = extractFromSentence("Our cloud customers include Microsoft, Amazon, Alphabet, Meta and Oracle.", "fixture");
  ...
  it("extracts named single-source supplier risk but ignores anonymous supplier risk", () => {
    const named = extractFromSentence("We depend on ASML as a sole supplier for lithography systems used in advanced semiconductor manufacturing.", "fixture", {
      subjectSurface: "ENT-INTEL"
    });
    const anonymous = extractFromSentence("We depend on a limited number of suppliers for critical components.", "fixture", { subjectSurface: "ENT-INTEL" });
